...
__global__ void __cuda_aes_4b_encrypt__(
    ...
    )
{
	...
	unsigned int p = bi;
	uint32_t sti = threadIdx.x/4; //state index
	...
	if(		   m == paracrypt::BlockCipher::CTR
		...
		if(valid_thread && ti == 0) {
			__LOG_TRACE__("p %d: d => 0x%04x",p,d[p]);
			s0[sti] ^= d[p];
...
__global__ void __cuda_aes_4b_decrypt__(
        const paracrypt::BlockCipher::Mode m,
		unsigned int n,
		unsigned int offset,
		const uint32_t* d,
    ...
    )
{
	...
	unsigned int p = bi;
	unsigned int sti = threadIdx.x/4; //state index
	int ti = threadIdx.x%4; // block-thread index: 0, 1, 2 or 3 (4 threads per cipher-block)
	int valid_thread = bi < n*4;
	unsigned int key_index_sum = 0;
	      ...
	      )
{
	...
	__cuda_aes_4b_encrypt__<<<gridSize,threadsPerBlock,shared_memory,stream>>>(//*2>>>(
			m,
			n_blocks,
			offset,
			(const uint32_t*)in,
...
{
	int shared_memory = threadsPerBlock*2*sizeof(uint32_t);
	__cuda_aes_4b_decrypt__<<<gridSize,threadsPerBlock,shared_memory,stream>>>(
			m,
			n_blocks,
			offset,
			(const uint32_t*)in,
