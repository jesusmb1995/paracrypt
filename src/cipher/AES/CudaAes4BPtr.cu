...
#include "CudaAes4BPtr.cuh"
#include "utils/cuda_logging.cuh"

__global__ void __cuda_aes_4b_ptr_encrypt__(
...
__global__ void __cuda_aes_4b_ptr_decrypt__(
        const paracrypt::BlockCipher::Mode m,
		unsigned int n,
		unsigned int offset,
		const uint32_t* d,
	      ...
	      )
{
	...
	__cuda_aes_4b_ptr_encrypt__<<<gridSize,threadsPerBlock,shared_memory,stream>>>(//*2>>>(
			m,
			n_blocks,
			offset,
			(const uint32_t*)in,
...
{
	int shared_memory = threadsPerBlock*2*sizeof(uint32_t);
	__cuda_aes_4b_ptr_decrypt__<<<gridSize,threadsPerBlock,shared_memory,stream>>>(
			m,
			n_blocks,
			offset,
			(const uint32_t*)in,
