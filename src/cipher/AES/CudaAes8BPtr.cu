...
__global__ void __cuda_ecb_aes_8b_ptr_encrypt__(
...
__global__ void __cuda_ecb_aes_8b_ptr_decrypt__(
		const paracrypt::BlockCipher::Mode m,
		unsigned int n,
		unsigned int offset,
		const uint32_t* d,
...
void cuda_ecb_aes_8b_ptr_encrypt(
		  	  int gridSize,
		  	  int threadsPerBlock,
		  	  cudaStream_t stream,
		  	  int n_blocks,
	      ...
	      )
{
	...
	__cuda_ecb_aes_8b_ptr_encrypt__<<<gridSize,threadsPerBlock,shared_memory,stream>>>(//*2>>>(
			n_blocks,
			(uint32_t*)in,
			(uint32_t*)out,
			(uint32_t*)neigh,
...
void cuda_ecb_aes_8b_ptr_decrypt(
		  	  int gridSize,
		  	  int threadsPerBlock,
		  	  cudaStream_t stream,
		  	  int n_blocks,
...
{
	int shared_memory = threadsPerBlock*4*sizeof(uint32_t);
	__cuda_ecb_aes_8b_ptr_decrypt__<<<gridSize,threadsPerBlock,shared_memory,stream>>>(
			n_blocks,
			(uint32_t*)in,
			(uint32_t*)out,
			(uint32_t*)neigh,
