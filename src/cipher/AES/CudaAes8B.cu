...
__global__ void __cuda_aes_8b_encrypt__(
    ...
    )
{
	...
	unsigned int p = bi*2;
	uint32_t sti = threadIdx.x/2; //state index / block index
...
__global__ void __cuda_aes_8b_decrypt__(
        const paracrypt::BlockCipher::Mode m,
		unsigned int n,
		unsigned int offset,
		const uint32_t* d,
	      ...
	      )
{
	...
	__cuda_aes_8b_encrypt__<<<gridSize,threadsPerBlock,shared_memory,stream>>>(//*2>>>(
			m,
			n_blocks,
			offset,
			(const uint32_t*)in,
...
{
	int shared_memory = threadsPerBlock*4*sizeof(uint32_t);
	__cuda_aes_8b_decrypt__<<<gridSize,threadsPerBlock,shared_memory,stream>>>(
			m,
			n_blocks,
			offset,
			(const uint32_t*)in,
